...
// complete attempt counts, taken names are refused before the password is hashed.
...
function signupPage(ctx, error, username = "", status = 200, kind = "error") {
...
async function signupSubmit(ctx) {
  ...
  if (wait) {
    ...
    return signupPage(ctx, `Too many sign-ups from this address; try again in ${minutes} minute${minutes === 1 ? "" : "s"}`, username, 429);
  }
  if (!username) return signupPage(ctx, "Enter a username", username, 400);
  const nameProblem = auth.usernameProblem(username);
  if (nameProblem) return signupPage(ctx, nameProblem, username, 400);
  const problem = auth.passwordProblem(password);
  if (problem) return signupPage(ctx, problem, username, 400);
  if (password !== str(form, "password2")) return signupPage(ctx, "The two passwords do not match", username, 400);
  ...
  if (await db.first(ctx.env, "SELECT 1 AS one FROM users WHERE username = ?", username)) {
    return signupPage(ctx, "That username is taken; pick another", username, 409);
  }
  const hash = await auth.hashPassword(password);
  let id;
  try {
    id = (await db.run(ctx.env, "INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)", username, hash, SIGNUP_ROLE)).last_row_id;
  } catch (e) {
    if (db.isConstraintError(e)) return signupPage(ctx, "That username is taken; pick another", username, 409); // lost the race
    throw e;
  }
  await auth.rotateSession(ctx, id);
  await audit.log(ctx, "user_signup", "user", id, { username, role: SIGNUP_ROLE }, { id, username });
