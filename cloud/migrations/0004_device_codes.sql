...
-- flasher collects it (the pending token is then cleared; the row stays and keeps counting
-- towards the per-IP rate limit until pruned). ip feeds the per-IP rate limit. Rows are
